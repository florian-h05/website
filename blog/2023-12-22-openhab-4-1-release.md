...
The standard text interpreter now understand your Item synonyms, 
and gives priority to an exact match on the label/synonym allowing an easier control over the command target.
It also understand your Item metadata descriptions, 
the rule `Set <ItemLabel> to <ItemCommandLabel>` is available out-of-the-box for the supported languages for all the Items with the commandDescription metadata.
...
* The `dialogGroup` option prevents simultaneous execution for dialogs on the same group,
so you can have near speakers and be sure those won't be triggered at the same time.
